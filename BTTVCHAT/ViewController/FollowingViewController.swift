...
class FollowingViewController: UIViewController {
    ...
    var channelsAPIURLStringTemplate = "https://api.twitch.tv/kraken/streams/followed?limit=50&stream_type=live&offset="
    var offset = 0
    var offsetInc = 25
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if index + 10 >= channels.count && !loadingChannels && !got0 {
            if let total = self.totalChannels, total > self.channels.count {
                getChannels(){ getCount in
                    if getCount > 0 {
                        DispatchQueue.main.async{
                            self.tableView.reloadData()
